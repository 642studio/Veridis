...
// Contrato Core: { status, lastEvent, updatedAt }
type SystemState = {
...
function buildSummary(state: SystemState): string {
  if (state.lastEvent) {
    ...
    if (ev.level === 'critical') {
      return `Critical alert: ${ev.message}`;
    }
    if (ev.level === 'warning') {
      return `Attention required: ${ev.message}`;
    }
    return `Info: ${ev.message}`;
  }
  if (state.status === 'processing') {
    return 'System is processing. Activity in progress.';
  }
  if (!state.lastEvent) {
    return 'System is idle. No recent activity.';
  }
  const ev = state.lastEvent;
  const type = 'type' in ev ? String(ev.type) : 'event';
  const source = 'source' in ev ? String(ev.source) : 'unknown';
  return `System is idle. Last event: ${type} from ${source}.`;
}

function buildSuggestedActions(state: SystemState): string[] {
  if (state.status === 'alert') {
    return ['Review alerts', 'Check event details'];
  }
  if (state.status === 'processing') {
    return ['Wait for completion', 'Check event stream'];
  }
  return ['Check system status', 'View recent events'];
}
    ...
    <main className="min-h-screen bg-zinc-950 text-zinc-300 font-mono text-sm p-6">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <header className="border-b border-zinc-800 pb-4">
          ...
          <p className="text-zinc-600 text-xs mt-0.5">642 Studio</p>
          <div className="mt-3 flex items-center gap-2">
            ...
            <span className="text-xs text-zinc-500">
              {error ?? (state ? 'Connected' : 'Connecting...')}
            </span>
          </div>
        </header>
        ...
        <section className="border border-zinc-800 rounded p-4 min-h-[120px]">
          ...
          {suggestedActions.length > 0 && (
            <div className="mt-3 pt-3 border-t border-zinc-800">
              <p className="text-zinc-600 text-xs mb-1">Suggested:</p>
            ...
            {state?.lastEvent ? (
              <li className="flex gap-3 text-zinc-500 py-0.5 border-b border-zinc-900/50">
                ...
                <span className="text-zinc-400">
                  {'type' in state.lastEvent ? String(state.lastEvent.type) : 'event'}
                </span>
              ...
              <li className="text-zinc-600 py-2">No events</li>
            )}
          </ul>
        </section>
      </div>
